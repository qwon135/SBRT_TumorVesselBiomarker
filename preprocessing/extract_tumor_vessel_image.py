...
save_root = f'SBRT_tumor_vessel_crop'

for pid in tqdm(os.listdir('data/SBRT_CT')):    
    try:
        os.makedirs(f'{save_root}/{pid}/', exist_ok=True)
        dcm = wrapper_from_file(
                    glob(f'data/SBRT_CT/{pid}/*.dcm')[0]
                    )
        vessel_mask_nib = nib.load(f'data/SBRT_CT_vessels/{pid}/lung_vessels.nii.gz')    
        ...
        images = get_3d_dicom(f'data/SBRT_CT/{pid}/')    
        tumor_mask = sorted(glob(f'data/SBRT_tumors/{pid}/*.png'))
        tumor_mask = np.stack([
